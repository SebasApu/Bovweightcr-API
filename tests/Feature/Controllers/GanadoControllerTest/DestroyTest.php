...
class DestroyTest extends TestCase
{
    ...
    public function test_usuario_puede_eliminar_ganado(): void
    {
        ...
        $response = $this->actingAs($usuario)
            ->deleteJson("/api/ganado/{$ganado->id}");

        $response->assertStatus(200)
            ->assertJsonPath('message', 'Animal eliminado correctamente');

        $this->assertDatabaseMissing('ganados', ['id' => $ganado->id]);
    }

    public function test_eliminar_ganado_inexistente_devuelve_404(): void
    {
        $usuario = User::factory()->ganadero()->create();

        $response = $this->actingAs($usuario)
            ->deleteJson('/api/ganado/9999');
